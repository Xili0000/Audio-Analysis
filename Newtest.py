...
def process_and_ask(mp3_path):
    ...
    # Conduct toxicity analysis of the entire text
    ...
    # Add overall toxicity analysis results
    if full_text_toxicity:
        output_text.append("Overall text toxicity analysis results：")
    ...
    for i, sentence in enumerate(sentences, 1):
        ...
        output_text.append(f"句子 {i}: {sentence}")
        output_text.append(f"AI判断结果 {i}: {response}")
        
        if sentence_toxicity:
            output_text.append(f"详细毒性分析 {i}:")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Using: python test.py <mp3 file path>")
        sys.exit(1)
